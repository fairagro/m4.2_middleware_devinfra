...
- Replace `scripts/setup-git-lfs.sh` with `scripts/setup-git-hooks.sh` (hooks only); thin stub at old name pointing to
  the new script (**B1**)
...
- `.devcontainer/Dockerfile`, `scripts/setup-git-hooks.sh`, stub `setup-git-lfs.sh`, `scripts/git-hooks/`,
