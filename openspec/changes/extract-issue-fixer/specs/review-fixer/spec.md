...
MUST be `Follow-up from PR #<n> AI review`. Low nits MUST NOT become issues. The skill MUST NOT use a separate
inline-only `gh issue create` template that bypasses create-issue. Relation MUST be `linked` by default (standalone
issue with Links to the PR). Relation MUST be `sub-of #<issue>` only when the PR body includes `Fixes #<issue>` (or
equivalent) and the deferred item is clearly remaining acceptance criteria of that issue. When unclear, prefer `linked`.
...
- **WHEN** the fixer opens a Medium+ follow-up that is not remaining acceptance criteria of a `Fixes #<n>` issue
