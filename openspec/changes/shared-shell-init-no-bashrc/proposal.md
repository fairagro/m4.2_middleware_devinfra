...
- Wire bash programmable completion for those short names in the **shared image**
  (`/usr/share/bash-completion/completions/{k,d}`) — not via `~/.bashrc`.
...
- Docs: remove guidance that implies bashrc/`load-env.sh` is the fleet pattern; point products at remoteEnv + wrappers +
  image completion + postCreate decrypt; note that product adopt (drop load-env/bashrc) is **out of this PR** via
  follow-up issues.
...
- `shared-devcontainer-base`: bashrc-free shell PATH contract, kubectl/docker wrappers **with image bash-completion**,
  optional postCreate SOPS decrypt to `.env`, consumer docs for product adopt without `load-env.sh`.

## Impact

- `.devcontainer/devcontainer.json` (Devinfra overlay), `.devcontainer/Dockerfile` (completions for `k`/`d`),
  `scripts/bin/*`, `scripts/devcontainer-post-create.sh`, `docs/synced-paths.yaml`, `docs/devcontainer.md` / conventions
  pointers, OpenSpec main sync on archive.
