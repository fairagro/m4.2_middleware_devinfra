...
- **issue-fixer OpenSpec:** when explore is required → `/opsx-explore`; then `/opsx-propose` → pause → `/opsx-apply` →
  pause → `/opsx-archive`. OpenSpec is **issue-fixer only** (not review-fixer / create-issue). Missing `openspec/` →
  stop.
