...
- **Bandit** MAY remain hooks + CI only in the shared IDE baseline (no required Bandit IDE extension settings). The fail
  bar MUST still match across hooks and CI (MEDIUM/HIGH fail; LOW never fails), even when CI logs LOW findings that
  hooks suppress with `-ll`.
- **Pylint `--source-roots`** MAY be omitted from the synced pre-commit entry. Product path overlays MUST stay on
  process env / reusable CI `pylint_source_roots`, not in synced `.pre-commit-config.yaml`. Documentation MUST state
  that this overlay is not the fail bar (`fail-under` on `.pylintrc`).
...
environment parity rule, the named exceptions, and the minimal-CLI rule.
...
- **THEN** each environment uses the same shared config file(s) for that tool (Bandit excepted in the IDE)
- **AND** the gate outcome (pass vs fail on policy findings) is the same across the three for tools that run in that
  environment
...
- **AND** Bandit remains the named hooks+CI-only exception
