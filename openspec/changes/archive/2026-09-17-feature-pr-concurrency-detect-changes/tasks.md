...
- [x] 1.1 Replace the partial Feature-PR jobs-only snippet in `docs/ci.md` with a complete recommended workflow
      ...
      YAML
- [x] 1.2 Add release / pre-release / Helm concurrency guidance (`cancel-in-progress: false`, serialize by workflow+ref)
      and state that `detect-changes` / `skip` is Feature-PR-oriented; verify the section is present next to those
      caller examples
- [x] 1.3 Document that reusable-level concurrency does not replace caller-level full-PR cancel and that `skip` /
      `detect-changes` stay caller-owned; verify `npm run lint:md` is clean for touched docs

## 2. Outer reusable concurrency

- [x] 2.1 Add `concurrency` (`cancel-in-progress: true`) to `reusable-code-quality.yml`, `reusable-build.yml`, and
...
- [x] 2.2 Add `concurrency` (`cancel-in-progress: false`) to `reusable-release.yml`, `reusable-helm-release.yml`,
...
- [x] 3.1 Run `openspec validate feature-pr-concurrency-detect-changes --strict` and fix until it passes
