...
`Refactoring`. It MUST NOT encode kinds as `kind:*` labels. It MUST attach exactly one allowlisted `severity:*` and one
`cost:*` label. It MUST attach a `practicality:*` label **only** when there is a concrete path to a bad outcome (typical
for `Bug` / `Security` and `/review-fixer` Medium+ deferrals); it MUST NOT attach `practicality:high` merely because the
work is implementable. When omitting practicality, the issue body triage MUST record
`practicality: n/a (no defect path)` (or equivalent). Allowed label names are only from:
