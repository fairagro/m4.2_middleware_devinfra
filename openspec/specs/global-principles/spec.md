...
overwritten). The document MUST cite examples including `principles.global.md` / `principles.md` and
`surface-quality-bar.global.md` / `surface-quality-bar.md`. It MUST NOT present import-linter as a `.global` pair
(product-owned `.importlinter` only). It MUST forbid inventing alternate suffixes such as `.product` for this split.
...
- **THEN** they find the synced-`.global` + product-local pair rule with principles / surface-quality-bar examples
