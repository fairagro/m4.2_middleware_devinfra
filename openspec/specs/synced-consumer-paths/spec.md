...
The repository MUST provide `docs/synced-paths.yaml` as the canonical allowlist of paths synced from Devinfra into
product repos that consumers MUST NOT hand-edit. The YAML MUST include an `allow` list (or equivalent) of concrete
paths/globs covering at least: shared AI review policy and surface-bar global map, fixer skill/command/prompt
entrypoints, first-party and vendor agent skills under `.agents/skills/` that sync ships, `scripts/ai/**`,
personal-token helpers (`scripts/dev-tokens.sh`, `scripts/set-dev-tokens.sh`, `scripts/bin/gh`, `scripts/bin/git`),
`openspec/principles.global.md`, **`renovate.json`**, **`.github/workflows/renovate.yml`**, and other Wave A /
sync-manifest paths as they exist. It MUST name documented **product-local overlay** exceptions (e.g. via `overlays` /
`exclude`, including `docs/surface-quality-bar.md`, `openspec/principles.md`, `AGENTS.md`) that MAY be edited in
consumers. The root `README.md` Docs index MUST link to this YAML. Sync of this allowlist MUST NOT
