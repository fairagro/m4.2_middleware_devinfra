...
Canonical reusable GitHub Actions workflows for shared code-quality, image/SBOM checks, Docker build/release, and Helm
publish so m4.2 product repos call Devinfra by `uses:` instead of copying YAML. PyPI and ns-pages remain product-local.
...
medium/high fail policy, **vulture**, **import-linter**, **uv audit** lockfile gate, pytest, and Prettier/markdownlint
check scripts) against a configurable package root (Python) plus repository Markdown via npm scripts. The dependency
install step MUST enable uv’s malware check (`UV_MALWARE_CHECK=1` or equivalent). It MUST accept a boolean `skip` input
that still runs the workflow job successfully with a no-op path when true (so required status checks are not left
pending). The default Code Quality job display name MUST remain `Code Quality Check (3.12)` for branch-ruleset
compatibility unless a later change explicitly migrates consumers.
