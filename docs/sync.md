...
Do **not** invent mypy config-merge here — third-party silence for fleet deps (`arctrl` /
`fable_library`) lives in synced `mypy.ini` module overrides; `MYPYPATH` remains the product
path overlay for first-party packages ([`docs/quality.md`](quality.md)).
